...
create_view
    : (CREATE (OR REPLACE)? | REPLACE) (OR? FORCE)? EDITIONING? VIEW
      tableview_name view_options?
      AS teradata_view_options* subquery order_by_clause? subquery_restriction_clause?
    ;

teradata_view_options
	: (LOCKING | LOCK) (ROW|tableview_name) FOR ACCESS
	;
...
create_table
    : CREATE (GLOBAL TEMPORARY)? (SET|MULTISET)? TABLE tableview_name teradata_options?
...
teradata_option
	: (identifier | DEFAULT)+
	| identifier '=' (expression  | DEFAULT| ALL) identifier?
	;
...
relational_table
    : ('(' relational_properties ')')?
      ...
      physical_properties? column_properties? primary_key_clause? table_partitioning_clauses?
      table_compression?
      (CACHE | NOCACHE)? (RESULT_CACHE '(' MODE (DEFAULT | FORCE) ')')?
      parallel_clause?
      (ROWDEPENDENCIES | NOROWDEPENDENCIES)? (MONITORING | NOMONITORING)?
      (enable_disable_clause+)? row_movement_clause? flashback_archive_clause?
    ;
...
column_definition
    : column_name (datatype | type_name) title?
         SORT?  (DEFAULT expression)?
         (ENCRYPT (USING  CHAR_STRING)? (IDENTIFIED BY regular_id)? CHAR_STRING? (NO? SALT)? )?
         (inline_constraint* | inline_ref_constraint) (DEFAULT expression)? compress_definition?
    ;
...
constraint_clauses
    : ADD '(' ( ','? (table_alias inline_constraint* | out_of_line_constraint) )+ ')'
...
primary_key_clause
    : UNIQUE? PRIMARY (KEY|INDEX) id_expression? paren_column_list using_index_clause?
    ;
...
fetch_statement
    : FETCH cursor_name (it1=INTO (','? into_element)+ | BULK COLLECT INTO (','? into_element)+)
    ;
...
select_statement
    : subquery_factoring_clause? subquery (for_update_clause | order_by_clause | offset_clause | fetch_clause)* limit_clause?
    ;
...
query_block
    : (SELECT | SEL) (DISTINCT | UNIQUE | ALL)? (TOP numeric)? (ASTERISK | (','? selected_element)+)
      into_clause? from_clause? where_clause? hierarchical_query_clause? (order_by_clause? group_by_clause)? where_clause? model_clause? qualify_clause?
    ;
...
table_ref_aux
    : table_ref_aux_internal flashback_query_clause* (/*{isTableAlias()}?*/AS? table_alias)? view_options?
    ;
...
compound_expression
    : concatenation
      (NOT? ( IN ANY? in_elements
            | BETWEEN between_elements
            | LIKE ANY '(' concatenation (',' concatenation)* ')' 
            | like_type=(LIKE | LIKEC | LIKE2 | LIKE4) concatenation (ESCAPE concatenation)? ) )?
    | concatenation MONTH '(' expression ')'
    | concatenation YEAR '(' expression ')'
    | compound_expression '(' type_spec ')'
    | compound_expression '(' format_spec ')'
    ;
...
simple_case_expression
    : label_name? ck1=CASE expression simple_case_when_part+  case_else_part? END CASE?
    ;
...
searched_case_expression
    : label_name? ck1=CASE searched_case_when_part+ case_else_part? END CASE?
    ;
...
atom
    ...
    | '(' expressions ')'
    | amp_variable outer_join_sign?
    ;

amp_variable
    : AMPERSAND UNSIGNED_INTEGER
    ;
...
other_function
    ...
    | COUNT '(' ( ASTERISK | (DISTINCT | UNIQUE | ALL)? concatenation) ')' over_clause?
    | (CAST | XMLCAST) '(' (MULTISET '(' subquery ')' | concatenation) AS (type_spec | format_spec) ')'
    ...
    | POSITION '(' concatenation IN concatenation ')'
...
into_clause
    : (BULK COLLECT)? INTO (','? into_element)+
    ;
...
datatype
    : native_datatype_element precision_part? (WITH LOCAL? TIME ZONE | CHARACTER SET char_set_name)? (NOT? CASESPECIFIC)? (FORMAT expression)?
...
constant
    ...
    | INTERVAL (quoted_string | bind_variable | general_element_part)
      ...
      (TO ( interval_type ('(' (UNSIGNED_INTEGER | bind_variable) ')')?))?
    | numeric
    | DATE quoted_string
    | quoted_string
    | NULL_
